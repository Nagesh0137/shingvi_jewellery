...
.payment-option input[type="radio"]:checked + label {
    background-color: ghostwhite; /* Light orange-like hover */
    border-color: #9c1137;
}
...
.modal.modal-bottom .modal-dialog {
  ...
  border-top-right-radius: 20px;
  width: 100%;
  height: 75%;  /* Makes the modal height 75% of screen */
  margin: 0 auto;
...
#modalOverlay {
  ...
  z-index: 1040; /* below modal (Bootstrap modal uses 1050) */
}
...
<form method="post" action="<?=base_url()?>user/save_buy_now" id="AddressModalForm">
    ...
    <div class="card p-2">
    	<div class="d-flex justify-content-between">
    		<div class="col-3" style="">
    			...
    			<img src="<?=base_url()?>uploads/<?=$product_details[0]['imgs'][0]?>" style="height: 100%;width: 100%;overflow: hidden;object-fit: cover;border: 0.1px solid lightgray;border-radius: 20px;">
    		</div>
    		<div class="col-9">
    			<div class="p-2">
    				<h6 class="font-15" style="color: darkcyan;"><?=$product_details[0]['category_name']?></h6>
    				<h6><?=$product_details[0]['product_name']?></h6>
    				<p class="clamp-text" style="font-size: 12px;">lorem<?=$product_details[0]['product_details']?></p>
	...
	<div class="address-modal-header bg-white p-2 mt-2 mb-1 rounded d-flex align-items-center justify-content-between meb-30">
        <h6 class="font-15">Order Summary (1 Item)</h6>
        <h6 id="final_amount_after_discount">&#8377; <?=number_format(floatval($product_details[0]['final_amount_after_discount']) * floatval($qty))?></h6>
	    <input type="hidden" name="product_id" value="<?=$product_id?>" id="product_id" class="product_id">
	    <input type="hidden" name="customers_id" value="<?=$user[0]['customers_id']?>" id="customers_id" class="customers_id">
    	...
    	if($userAddressStatus == 'New')
    	{
    		...
    		<div class="card p-1 mt-2">
		    	<div class="d-flex p-2 align-items-center justify-content-between" style="vertical-align:middle;">
			    	...
			    	<button type="button" class="btn col-6 justify-content-end btn-sm text-white float-end" style="background:#9c1137" onclick="openModal()">+ Add Address</button>
		    	</div>
	    	</div>
    		<?php
    	}else{
    ...
    <div class="card p-1 mt-2">
	    <div class="payment-option">
	        ...
	        <label class="payment-label" for="online">
	            <img src="https://cdn-icons-png.flaticon.com/512/10551/10551890.png" style="width: 20px;"><span>Online Payment</span>
	        </label>
	    </div>
	    ...
	    <div class="payment-option">
	        ...
	        <label class="payment-label" for="cod">
	                <img src="https://cdn-icons-png.flaticon.com/512/9198/9198191.png" style="width: 20px;"><span>Cash on Delivery</span>
	        </label>
	    </div>
...
<form method="post" action="<?=base_url()?>user/save_buy_now" id="AddressModalForm">
    ...
    <div class="address-modal-header bg-white p-2 rounded d-flex align-items-center justify-content-between meb-30">
        ...
        <button type="button" onclick="showProductDetails('<?=$product_details[0]['prod_gold_id']?>')" class="body-secondary-color icon-16"><i class="ri-arrow-right-line d-block lh-1"></i></button>
    </div>
    ...
    <div class="address-modal-form mt-2">
        <div class="row field-row">
            ...
            <input type="hidden" name="customers_id" class="customers_id">
...
<div id="addressModal" class="modaladdress" style="position: fixed;bottom: 0;box-shadow: rgba(17, 17, 26, 0.1) 0px 4px 16px, rgba(17, 17, 26, 0.1) 0px 8px 24px, rgba(17, 17, 26, 0.1) 0px 16px 56px;">
  <div  class="modal-dialog modal-fullscreen-sm-down modal-dialog-scrollable modal-bottom my-0 justify-content-center">
    <div class="modal-content bg-white p-3" style="animation: slideUp 0.3s ease-out; border-top-left-radius: 20px;border-top-right-radius: 20px;">
      ...
      <div id="savedAddressesSection" class="pb-5" >
        ...
        <h5>Saved Addresses</h5>
      	...
      	<div class="row mt-0 pt-0">
        	...
        	<button class="btn col-6 justify-content-end btn-sm text-white float-end" style="background:#9c1137" onclick="toggleForm()">+ Add address</button>
      	</div>
      		<?php 
        	...
        	<div class="address-box d-flex mb-1 p-1 border">
          		<input onchange="useThisAddress(this,'<?=$product_id?>')" value="<?=$row['customer_address_id']?>" type="radio" name="address" id="address1" <?=($row['default_address']=='yes') ? 'checked' : '';?>>
      ...
      <!-- Add Address Form -->
      <div id="addAddressSection" style="display:none; border-top-left-radius: 20px;border-top-right-radius: 20px;">
      	<div class="d-flex justify-content-between align-items-center">
        ...
        <div class="row field-row">
            ...
            <input type="hidden" value="<?=$_SESSION['user_id']?>" name="customers_id" class="customers_id">

            
            <div class="col-6 field-col">
                <label for="pincode" class="field-label">Pincode</label>
                <input type="number" id="new_pincode" name="pincode" class="w-100" placeholder="Enter Pincode" required>
            </div>
            <div class="col-6 field-col">
                <label for="city" class="field-label">City</label>
                <input type="text" id="new_city" name="city" class="w-100" placeholder="Enter City" required>
            </div>
            <div class="col-12 field-col">
                <label for="uaddress" class="field-label">Address</label>
                <input type="text" id="new_address" name="address" class="w-100" placeholder="Enter Address" required>
            </div>
        </div>

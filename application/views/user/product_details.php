<main id="main">
    <!-- <?php
                                ...
                                <div class="row btn-row15">
                                    ...
                                    <div class="col-12 col-sm-6">
                                        ...
                                        if (isset($_SESSION['user_id'])) {
                                            ?>
                                            <a onclick="openAddressModalpd('<?= $products[0]['prod_gold_id'] ?>')"
                                                class="w-100 btn-style secondary-btn">Buy now</a>
                                        <?php } else { ?>

                                            <a onclick="showBuyNowModal('<?= $products[0]['prod_gold_id'] ?>')"
                                                class="w-100 btn-style secondary-btn">Buy now</a>
                                        <?php } ?>
                                    </div>
                                </div>
        ...
        <script>
            ...
            // Add CSS for new user registration form
            const newUserFormStyles = `
                <style>
                #newUserRegistrationForm {
                    animation: fadeIn 0.3s ease-in;
                }
                
                #newUserRegistrationForm .field-col {
                    margin-bottom: 15px;
                }
                
                #newUserRegistrationForm input,
                #newUserRegistrationForm textarea {
                ...
                #newUserRegistrationForm input:focus,
                #newUserRegistrationForm textarea:focus {
                    border-color: #9c1137;
                    outline: none;
                    box-shadow: 0 0 5px rgba(156, 17, 55, 0.3);
                }
                
                #newUserRegistrationForm .field-label {
                    font-weight: 600;
                    color: #333;
                    margin-bottom: 5px;
                    display: block;
            ...
            // Add styles to head
            if (!document.querySelector('#newUserFormStyles')) {
                const styleElement = document.createElement('div');
                styleElement.id = 'newUserFormStyles';
                styleElement.innerHTML = newUserFormStyles;
            ...
            $(document).on('submit', '#otpForm', function (e) {
                ...
                if (inputOtp == receivedOtp) {
                    ...
                    if (user_status === 'existing') {
                        // Existing user - set session and proceed
                        $.ajax({
                            ...
                            success: function (res) {
                                console.log(res);
                                if (res.status === 'success') {
                                    console.log('Existing User Session Set');
                                    ...
                                    // Close buy-now modal and open address modal
                                    $('#buy-now-modal').modal('hide');

                                    // Wait for the modal to fully hide before opening address modal
                                    $('#buy-now-modal').on('hidden.bs.modal', function () {
                                        $(this).off('hidden.bs.modal'); // Remove event to prevent multiple calls
                                        openAddressModalpd(productDet.prod_gold_id);
                                    });
                                } else {
                                    console.log("Failed To Set User In Session");
                                    document.getElementById('errMsg').innerHTML = 'Failed to set user session. Please try again.';
                                }
                            },
                            error: function () {
                                document.getElementById('errMsg').innerHTML = 'Error setting user session. Please try again.';
                            }
                        });
                    } else if (user_status === 'new') {
                        // New user - show registration form within the modal
                        console.log('New User - Show Registration Form');
            ...
            // Function to show new user registration form
            function showNewUserRegistrationForm() {
                // Hide OTP form and show registration form
                $('#otpForm').hide();

                // Create and show the new user registration form
                const registrationFormHtml = `
                    <div id="newUserRegistrationForm">
                        <div class="buy-now-modal-form">
                            <div class="row field-row">
                                <div class="col-12 text-center mb-3">
                                    <h5 style="color: #9c1137;">Complete Your Registration</h5>
                                    <p class="text-muted">Please provide your details to continue</p>
                                </div>
                                <input type="hidden" name="product_id" value="${productDet.prod_gold_id}" class="product_id">
                                <input type="hidden" name="mobile_number" value="${enteredMobile}" id="new_user_mobile">
                                
                                <div class="col-12 col-md-6 field-col">
                                    <label for="new_user_name" class="field-label">Full Name *</label>
                                    <input type="text" id="new_user_name" name="name" class="w-100" 
                                           placeholder="Enter your full name" required>
                                </div>
                                
                                <div class="col-12 col-md-6 field-col">
                                    <label for="new_user_email" class="field-label">Email *</label>
                                    <input type="email" id="new_user_email" name="email" class="w-100" 
                                           placeholder="Enter your email" required>
                                </div>
                                
                                <div class="col-12 col-md-6 field-col">
                                    <label for="new_user_pincode" class="field-label">Pincode *</label>
                                    <input type="number" id="new_user_pincode" name="pincode" class="w-100" 
                                           placeholder="Enter pincode" required>
                                </div>
                                
                                <div class="col-12 col-md-6 field-col">
                                    <label for="new_user_city" class="field-label">City *</label>
                                    <input type="text" id="new_user_city" name="city" class="w-100" 
                                           placeholder="Enter city" required>
                                </div>
                                
                                <div class="col-12 field-col">
                                    <label for="new_user_address" class="field-label">Address *</label>
                                    <textarea id="new_user_address" name="address" class="w-100" rows="3"
                                              placeholder="Enter your complete address" required></textarea>
                                </div>
                                
                                <div class="col-12 field-col">
                                    <span id="newUserErrMsg" class="text-danger"></span>
                                </div>
                            </div>
                            
                            <div class="question-form-btn mst-20 mst-sm-30">
                                <button type="button" id="registerNewUserBtn" class="w-100 btn-style secondary-btn">
                                    Complete Registration & Continue
                                </button>
                            </div>
                            
                            <div class="question-form-btn mst-10">
                                <button type="button" id="backToOtpBtn" class="w-100 btn-style quaternary-btn">
                                    Back to OTP
                                </button>
                            </div>
                        </div>
                ...
                // Append the registration form after the OTP form
                $('#otpForm').after(registrationFormHtml);

                // Auto-populate pincode from API
                $('#new_user_pincode').on('input', function () {
                        ...
                        fetch(`https://api.postalpincode.in/pincode/${pincode}`)
                            ...
                            .then(data => {
                                if (data[0].Status === 'Success') {
                                    $('#new_user_city').val(data[0].PostOffice[0].District);
                                }
                            })
                ...
                $('#registerNewUserBtn').on('click', function () {
                    const name = $('#new_user_name').val().trim();
                    const email = $('#new_user_email').val().trim();
                    const pincode = $('#new_user_pincode').val().trim();
                    const city = $('#new_user_city').val().trim();
                    const address = $('#new_user_address').val().trim();

                    // Basic validation
                    if (!name || !email || !pincode || !city || !address) {
                        $('#newUserErrMsg').text('Please fill all required fields.');
                        return;
                    }

                    if (pincode.length !== 6) {
                        $('#newUserErrMsg').text('Please enter a valid 6-digit pincode.');
                        return;
                    }
                    ...
                    if (!emailRegex.test(email)) {
                        $('#newUserErrMsg').text('Please enter a valid email address.');
                        return;
                    }

                    $('#newUserErrMsg').text('');
                    $('#registerNewUserBtn').prop('disabled', true).text('Registering...');

                    // Register new user
                    $.ajax({
                        url: '<?= base_url("user/register_new_user") ?>',
                        type: 'POST',
                        data: {
                            ...
                            city: city,
                            address: address,
                            product_id: productDet.prod_gold_id
                        },
                        dataType: 'json',
                        success: function (res) {
                            console.log('New user registration response:', res);
                            if (res.status === 'success') {
                                // Set user details for the address modal
                                userDet = res.user_data;
                                user_status = 'existing'; // Now treat as existing user

                                $('#newUserRegistrationForm').remove();
                                $('#otpForm').show();

                                // Show success message
                                document.getElementById('errMsg').innerHTML = '<span style="color: green;">Registration successful! Opening order form...</span>';

                                // Wait a moment then close buy-now modal and open address modal
                                setTimeout(function () {
                                    $('#buy-now-modal').modal('hide');
                                ...
                                $('#newUserErrMsg').text(res.message || 'Registration failed. Please try again.');
                                $('#registerNewUserBtn').prop('disabled', false).text('Complete Registration & Continue');
                            }
                        },
                        error: function () {
                            $('#newUserErrMsg').text('Error occurred during registration. Please try again.');
                            $('#registerNewUserBtn').prop('disabled', false).text('Complete Registration & Continue');
                        }
                    });
                });

                // Handle back to OTP button
            ...
            // Function to open address modal for product purchase
            function openAddressModalpd(pId) {
                console.log('Opening address modal for product:', pId);

                let selected_qty = $('#selected_qty').val() || 1;
                let selected_size = $('input[name="selected_size"]:checked').val() || 'NA';

                var url = '?pId=' + encodeURIComponent(pId) +
                    '&size=' + encodeURIComponent(selected_size) +
                    '&qty=' + encodeURIComponent(selected_qty);

                $('#address-modal-body').load('<?= base_url("user/load_address_form") ?>' + url, function (response, status, xhr) {
                    if (status === 'success') {
                        console.log('Address modal content loaded successfully');
                        $('#address-modal').modal('show');
                    } else {
                        console.error('Failed to load address modal content');
                        document.getElementById('errMsg').innerHTML = 'Error loading order form. Please try again.';
                    }
                });
            }

            // Function for adding related products to cart (no size selection needed)
            function addToCartRelated(pId) {
                event.preventDefault();

                // For related products, we can use a default size or "NA"
                var selectedSize = "NA"; // or you can set a default size
                var selectedQty = 1; // Default quantity for related products
            ...
            // Function to show buy now modal for non-logged-in users
            function showBuyNowModal(pId) {
                console.log('Showing buy now modal for product:', pId);

                // Set the product ID for the OTP form
                $('#product_id').val(pId);
                ...
                // Reset OTP form visibility
                $('#otp').closest('.field-col').addClass('d-none');
                $('.question-form-btn').eq(0).removeClass('d-none'); // Show "Send OTP"
                $('.question-form-btn').eq(1).addClass('d-none'); // Hide "Verify"

<div class="container-fluid">
	...
	<div class="row">
		<div class="col-md-12">
            <div class="card">
                <div class="card-body">
					<form action="<?= base_url() ?>Madmin/order_delivered" method="get">
                        <div class="col-sm-12">
				...
				<?php $i=$start; if(isset($order) && count($order)>0){foreach ($order as  $value) { ?>
					<tbody>
						<tr>
							<td><?=++$i;?></td>
							<td><?=$value['name'];?></td>
							...
							<td><?= date('d M Y ',strtotime($value['delivered_date']));?></td>
							<td><a href="<?=base_url();?>Madmin/order_delivered_view?id=<?=$value['user_billing_details_id'];?>"><button class="btn btn-success"><i class="fa fa-eye"></i></button></a></td>
						</tr>   
					</tbody>
				<?php  } }else {?>
					<tfoot>
						<tr>
							<td colspan="8"><h1 class="text-center text-danger pt-3 pb-2">Currently No Delivered Orders</h1></td>
						</tr>
					</tfoot>
				<?php } ?>
			</table>

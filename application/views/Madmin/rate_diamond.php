<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-xl-6">
            <div class="card">
                <div class="card-body">
                    <form action="<?=base_url()?>Madmin/diamond_rate_add" method="post" enctype="multipart/form-data" id="form1">
                        <div class="mb-3">
                            <label for="formrow-firstname-input" class="form-label">Enter Todays Rate(10g)</label>
                            <input type="text" name="diamond_amt" id="diamond_amt" placeholder="Todays Price" required class="form-control" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" >
                        </div>
                    ...
                    <form action="<?=base_url()?>Madmin/diamond_rate_update" method="post" enctype="multipart/form-data" id="form2">
                        <div class="mb-3">
                            <label for="formrow-firstname-input" class="form-label">Enter Todays Rate(10g)</label>
                            <input type="text" name="diamond_amt" id="diamond_amt1" placeholder="Todays Price" required  class="form-control" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" style="width:480px">
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="formrow-email-input" class="form-label">Enter Todays Rate Date</label>
                                    <input type="hidden" name="rate_diamond_id" id="rate_diamond_id1" required class="form-control">
        ...
        <div class="col-xl-6">
            <div class="card">
                <div class="card-body">
                <form style="border-bottom:1px solid black;" action="<?=base_url()?>Madmin/diamond_rate_search" method="get" enctype="multipart/form-data">
                        <div class="row">
                            <div class="col-md-5">
                                <div class="mb-3">
                                    <label for="formrow-email-input" class="form-label">From Date</label>
                                    <input type="date" name="from_date" value="<?= $this->input->get('from_date') ?>" class="form-control" title="From Date" required>
                                </div>
                            </div>
                            <div class="col-md-5">
                                <div class="mb-3">
                                    <label for="formrow-password-input" class="form-label">To Date</label>
                                    <input type="date" value="<?= $this->input->get("to_date") ?>" name="to_date" class="form-control" title="To Date" required>
                                </div>
                            </div>
                            ...
                            <tbody>
                                <?php $i=1; foreach ($rate as $value) { ?>
                                    <tr>
                                        ...
                                        <!-- <button class="btn btn-outline-primary py-1 px-2" onclick="updatediamond('<?=$value['rate_diamond_id'];?>','<?=$value['ratedate'];?>','<?=$value['ratetime'];?>','<?=$value['diamond_amt'];?>','<?php if(isset($search['from_date'])){echo $search['from_date'];}?>','<?php if(isset($search['to_date'])){echo $search['to_date'];}?>')"><i class="fa fa-edit"></i></button> -->
                                        <a href="<?= base_url() ?>Madmin/edit_diamond_rate/<?= $value['rate_diamond_id'] ?>" class="btn btn-outline-primary py-1 px-2"><i class="fa fa-edit"></i></a>

                                            <a onclick="return confirm('Are You Sure to permanant Delete this data...')" href="<?=base_url();?>Madmin/diamond_rate_delete/<?=$value['rate_diamond_id'];?><?php if(isset($search['from_date'])){echo "/".$search['from_date'];}?><?php if(isset($search['to_date'])){echo "/".$search['to_date'];}?>" class="btn btn-outline-danger py-1 px-2"><i class="fa fa-trash"></i></a>

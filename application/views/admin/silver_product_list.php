		...
		<div class="col-md-12">
			<div class="card">
				<div class="col-md-12 p-2">
					<form action="<?= base_url(); ?>admin/search_silver_product_list" method="get">
						<div class="row">
				...
				<div class="card-body">
					<div class="row">
						<div class="col-md-6">
							...
							if (isset($_GET['q']) && !empty($_GET['q']) || isset($_GET['cat_id']) && !empty($_GET['cat_id']) || isset($_GET['group_id']) && $_GET['group_id'] != 'all') {
								// If any of the search parameters are set, show the download button
								?>
								<a href="<?= base_url() ?>admin/download_silver_csv?cat_id=<?= $_GET['cat_id'] ?? '' ?>&group_id=<?= $_GET['group_id'] ?? '' ?>&q=<?= $_GET['q'] ?? '' ?>"
									class="btn btn-success btn-sm">Download CSV</a>
							<?php } else { ?>
								<a href="<?= base_url() ?>admin/download_silver_csv2"
									class="btn btn-success btn-sm">Download CSV</a>
							<?php } ?>
						</div>
						<div class="col-md-6  float-end">

							<!-- This will always be floated to the right side -->
						...
						<table class="table text-center mt-3 table-sm table-bordered cat_table mb-5 table-hover">
							...
							<tbody>
									...
									if ($group_cat[0]['category_name'] == "Silver") {
										...
										<tr>
											...
											<td style="font-size: 16px;"
												class="d-flex aling-items-center justify-content-around <?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
								...
								if ($i == 0) {
									?>
									<tr>
										<td colspan="20" class="bg-danger">No Record Found</td>
									</tr>
									<?php
								}
								?>
...
<script>
	...
	document.getElementById('deleteAllButton').addEventListener('click', function () {
		...
		if (selectedProducts.length > 0 && confirm('Are you sure you want to delete the selected products?')) {
			...
			window.location.href = '<?= base_url(); ?>admin/silver_product_list_delete_all/' + encodedProducts;
		} else {

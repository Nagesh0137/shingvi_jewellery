		...
		<div class="col-md-12">
			<div class="card">
				<div class="col-md-12 p-2">
					<form action="<?= base_url(); ?>admin/search_gold_product_list" method="get">
						<div class="row">
							<div class="col-md-4 mt-2">
								<select name="cat_id" onchange="group_name(this)" id="cat_id" class="form-select"
									required>
				...
				<div class="card-body">
					...
					<table class="table text-center mt-3 table-sm table-bordered cat_table mb-5 table-hover">
						<thead>
							<tr>
								<th>
									Choose To Delete
									<br>
									<button class="btn btn-danger btn-sm ">Select All</button>
								</th>
						...
						<style>
							...
							foreach ($products as $row2) {
								...
								$i++;
								if (strtolower($group_cat[0]['category_name']) == "gold") {
									?>
										<tr>
											<td class="">
											...
											<td style="font-size: 16px;" class="d-flex aling-items-center justify-content-around <?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
												<a href="<?= base_url(); ?>admin/silver_product_list_View/<?= $row2['prod_gold_id']; ?>" class="btn btn-success p-0 m-0" style="height:3vh;width:3vh"><i class="fa fa-print"></i></a>
												&nbsp;
												<a href="<?= base_url(); ?>admin/silver_product_list_update/<?= $row2['prod_gold_id']; ?>" class="btn btn-primary p-0 m-0" style="height:3vh;width:3vh"><i class="fa fa-edit"></i></a>
												&nbsp;
												<a href="<?= base_url(); ?>admin/silver_product_list_delete/<?= $row2['prod_gold_id']; ?>" onclick="return confirm('Are You Sure...')" class="btn btn-danger p-0 m-0" style="height:3vh;width:3vh"><i class="fa fa-trash"></i></a> 
											...
											<td class="<?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
												&#8377;&nbsp;<?= number_format($jadmin->silverprice($row2['prod_gold_id'])); ?>&nbsp;/-
											</td>
											<td class="<?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
					...
					<div class="card-body">
						<div class="row">
							<div class="col-md-6">
								<a href="<?= base_url() ?>admin/download_gold_csv?q=<?= isset($_GET['q']) ? $_GET['q'] : '' ?>&cat_id=<?= isset($_GET['cat_id']) ? $_GET['cat_id'] : '' ?>&group_id=<?= isset($_GET['group_id']) ? $_GET['group_id'] : 'all' ?>"
									class="btn btn-success btn-sm">Download CSV</a>

							</div>
							<div class="col-md-6  float-end">

								<button id="deleteAllButton" class="btn btn-danger btn-sm float-end">Delete Selected
									Products</button>
							</div>
						</div>
					</div>
										...
										<tr>
											<td class="text-center">
												<input type="checkbox" class="productCheckbox"
													value="<?= $row2['prod_gold_id']; ?>" />
											</td>
											<td style="font-size: 16px;"
												class="d-flex align-items-center justify-content-around <?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
												<a href="<?= base_url(); ?>admin/silver_product_list_View/<?= $row2['prod_gold_id']; ?>"
													class="btn btn-success p-0 m-0" style="height:3vh;width:3vh"><i
														class="fa fa-print"></i></a>
												&nbsp;
												<a href="<?= base_url(); ?>admin/silver_product_list_update/<?= $row2['prod_gold_id']; ?>"
													class="btn btn-primary p-0 m-0" style="height:3vh;width:3vh"><i
														class="fa fa-edit"></i></a>
												&nbsp;
												<a href="<?= base_url(); ?>admin/silver_product_list_delete/<?= $row2['prod_gold_id']; ?>"
													onclick="return confirm('Are You Sure...')" class="btn btn-danger p-0 m-0"
													style="height:3vh;width:3vh"><i class="fa fa-trash"></i></a>
											</td>
											<td
												class="<?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
												<?= $i; ?>
											</td>
											<td
												class="<?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
												<button onclick="add_product_label(<?= $row2['prod_gold_id'] ?>)"
													class="btn btn-primary btn-sm pl-0 pr-0 pt-0 pb-0">&nbsp;&nbsp;ADD&nbsp;LABEL&nbsp;&nbsp;</button>
											</td>
											<td class="<?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>"
												...
												if ($row2['special_days_id'] != null) {
													...
													if (count($special_id) > 0 && !empty($special_id)) {
														...
														foreach ($special_id as $key3 => $row3) {
															...
															?>
															<button
																class="btn <?= $color_class ?> btn-sm pl-0 pr-0 pt-0 pb-0 my-1 text-capitalize"
																onclick="remove_special_day_offer(<?= $row2['prod_gold_id'] ?>, <?= $row3 ?>)">
																<i class="ri-close-line fw-bold">
																	<?= getspecialdays($row3); ?>
																</i>
											...
											<td class="<?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>"
												...
												if ($row2['label'] == "") {
													...
													foreach ($label as $key4 => $row4) {
														?>
														<button
															onclick="remove_product_labels(<?= $row2['prod_gold_id'] ?>,`<?= $row4 ?>`)"
															class="btn btn-danger btn-sm pl-0 pr-0 pt-0 pb-0 my-1">
															&nbsp;<i class="ri-close-line"></i>&nbsp;</i>
															<?= $row4 ?>&nbsp;&nbsp;&nbsp;
														</button>
														<?php
											...
											<td
												class="text-center <?= (stripos(trim($row2['label']), 'Out Of Stock') !== false) ? 'out-of-stock' : '' ?>">
												<?php
												$aa = explode('||', $row2['product_image']);
												$iimg = base_url() . 'uploads/' . $aa[0];
												?>
												<img src="<?= $iimg; ?>" style="height:100px;width:100px;"
													title="<?= $row2['product_id'] ?>">
											</td>
										</tr>
										<?php
									}
					...
					<script>
						...
						document.getElementById('deleteAllButton').addEventListener('click', function () {
							...
							if (selectedProducts.length > 0 && confirm('Are you sure you want to delete the selected products?')) {
								...
								window.location.href = '<?= base_url(); ?>admin/gold_product_list_delete_all/' + encodedProducts;
							} else {
		...
		<div class="modal-content p-5">
			...
			<div class="row d-flex justify-content-around align-items-center">
				...
				<div class="col-md-6">
					...
					<button class="mr-2 m-2 btn btn-danger lbl_btn" id="New"
						onclick="add_product_label2('New')">NEW</button>
					<button class="mr-2 m-2 btn btn-danger lbl_btn" id="New"
						onclick="add_product_label2('wedding')">Wedding</button>
					<button class="mr-2 m-2 btn btn-primary lbl_btn" id="Top-Selling"
						onclick="add_product_label2('Top Selling')">TOP SELLING</button>
					<button class="mr-2 m-2 btn btn-primary lbl_btn" id="Trending"
						onclick="add_product_label2('Trending')">TRENDING</button>
					<button class="mr-2 m-2 btn btn-warning lbl_btn" id="Sale"
						onclick="add_product_label2('Sale')">SALE</button>
					<button class="mr-2 m-2 btn btn-success lbl_btn" id="Special"
						onclick="add_product_label2('Special')">SPECIAL</button>
					<button class="btn btn-danger lbl_btn" id="Out-Of-Stock"
						onclick="add_product_label2('Out Of Stock')">OUT OF STOCK</button>
					<button class="btn btn-primary lbl_btn" id="Execlusive-Collection"
						onclick="add_product_label2('Execlusive Collection')">Execlusive Collection</button>
					<br>
					<button class="mr-2 m-2 btn btn-secondary lbl_btn" id="Gift" onclick="add_product_label2('Gift')">
						<i class="fa fa-gift" style="font-size: 25px;"></i><br> Gift</button>

					<button class="mr-2 m-2 btn btn-info lbl_btn" id="Coin" onclick="add_product_label2('Coin')"> <svg
							aria-hidden="true" focusable="false" data-prefix="fas" data-icon="coins" role="img"
							xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
							class="svg-inline--fa fa-coins fa-w-16 fa-2x">
...
<script type="text/javascript">
	...
	function add_product_label2(label, id = "") {
		...
		$.ajax({
			url: `<?= base_url() ?>admin/add_product_label`,
			type: 'POST',
			data: {
				'prod_gold_id': id,
				'label': label
	...
	function add_special_days(special_day_id, id = "") {
		...
		$.ajax({
			url: `<?= base_url() ?>admin/add_special_day_offer`,
			dataType: 'json',
			type: 'POST',
			data: {
	...
	function remove_special_day_offer(prod_gold_id, special_day_id) {
		$.ajax({
			url: '<?= base_url() ?>admin/remove_special_product',
			type: 'POST',
			data: {
	...
	function remove_product_labels(prod_gold_id, label) {
		$.ajax({
			url: `<?= base_url() ?>admin/remove_product_labels`,
			data: {
				'prod_gold_id': prod_gold_id,
				'label': label
			},
